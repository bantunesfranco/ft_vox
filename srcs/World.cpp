...
void World::generateChunkGreedyMesh(Chunk& chunk, const ChunkCoord& coord)
{
    ...
    for (int x = 0; x < W; ++x)
        for (int y = 0; y < H; ++y)
            for (int z = 0; z < D; ++z)
            {
                ...
                solid[x + 1][y + 1][z + 1] = isActive(v);
                blockTypes[x + 1][y + 1][z + 1] = getBlockType(v);
            }
    ...
    for (int y = 0; y < H; ++y)
        ...
        {
            // X neighbors
            Voxel vL = TerrainGenerator::sampleVoxel(baseWX - 1, y, baseWZ + z);
            solid[0][y + 1][z + 1] = isActive(vL);
            blockTypes[0][y + 1][z + 1] = getBlockType(vL);

            Voxel vR = TerrainGenerator::sampleVoxel(baseWX + W, y, baseWZ + z);
            solid[W + 1][y + 1][z + 1] = isActive(vR);
            blockTypes[W + 1][y + 1][z + 1] = getBlockType(vR);
        }
    ...
    for (int x = 0; x < W; ++x)
        ...
        {
            // Z neighbors
            Voxel vB = TerrainGenerator::sampleVoxel(baseWX + x, y, baseWZ - 1);
            solid[x + 1][y + 1][0] = isActive(vB);
            blockTypes[x + 1][y + 1][0] = getBlockType(vB);

            Voxel vF = TerrainGenerator::sampleVoxel(baseWX + x, y, baseWZ + D);
            solid[x + 1][y + 1][D + 1] = isActive(vF);
            blockTypes[x + 1][y + 1][D + 1] = getBlockType(vF);
        }
    ...
    for (int axis = 0; axis < 3; ++axis)
    {
        ...
        for (int dir = -1; dir <= 1; dir += 2)
        {
            ...
            struct MaskEntry { bool solid; uint8_t blockType; };
            ...
            for (x[axis] = -1; x[axis] < dims[axis]; ++x[axis])
            {
                ...
                for (x[v] = 0; x[v] < dims[v]; ++x[v])
                    for (x[u] = 0; x[u] < dims[u]; ++x[u])
                    {
                        ...
                        bool currentSolid = solid[cx + 1][cy + 1][cz + 1];

                        int nxi = std::clamp(nx + 1, 0, W + 1);
                        int nyi = std::clamp(ny + 1, 0, H + 1);
                        int nzi = std::clamp(nz + 1, 0, D + 1);
                        bool neighborSolid = solid[nxi][nyi][nzi];

                        uint8_t bt = blockTypes[cx + 1][cy + 1][cz + 1];

                        mask[n].solid = currentSolid && !neighborSolid && bt != 0;
                        mask[n].blockType = bt;
                ...
                for (int j = 0; j < dims[v]; ++j)
                {
                    ...
                    {
                        if (!mask[n].solid) { ++i; ++n; continue; }

                        int w = 1;
                        const uint8_t blockType = mask[n].blockType;

                        while (i + w < dims[u] && mask[n + w].solid && mask[n + w].blockType == blockType) ++w;

                        int h = 1;
                        for (; j + h < dims[v]; ++h)
                        {
                            for (int k = 0; k < w; ++k)
                                if (!mask[n + k + h * dims[u]].solid || mask[n + k + h * dims[u]].blockType != blockType)
                                    goto done_merge;
                        }
                        done_merge:
